...
# Settings#############################################################################################################################################
UnetTrain_dir <- "/home/gendarme/Documents/UNet/PMI_Data/Train/"
base_dir <- "/home/gendarme/Documents/UNet/PMI_Data/IF012_plate001_001"
...
currenti = paste0("noBorderDiamond11_thickness",THICKNESS)
...
train_data <- data.frame(ImageId = ImageTrain$ImageId) %>%
  ...
  mutate(ImageFile = file.path(base_dir, paste0(ImageId, ".tif")),
         ...
         Plate = ImageFile,
         Plate = sub(".*_plate001_", "", Plate),
         Plate = as.numeric(sub("/data.*", "", Plate)),
...
ImageProcessingAndEncoding <- function(Image, Plate, Well, Position){
  ...
  for (i in 1:(max(cytoMask) + 1)) {
    cytoMaskTemp <- erode(cytoMask == i, kern = makeBrush(THICKNESS, shape = "disc"))
  ...
  writeImage(Segm, paste0(segm_dir, "/", "p_",Plate,"--W_",Well,"--P_",Position,"--Segm.tif"), type = "tiff")
  ...
  n_sample_cyto <- as.tibble(train_data[Boolean & train_data$Channel == "mCherry",]) %>% 
    add_column(BorderMasks = list(as.array(ifelse(InterfaceBorderCyto == 2, 1, 0))),
               InterfaceMasks = list(as.array(ifelse(InterfaceBorderCyto == 3, 1, 0))),
  ...
  ## Export encoding
  write_csv(n_sample_out_nuc, paste0(UnetTrain_dir,"Plate_", Plate, "--Well_", Well, "--Position_", Position,"--Angle_",ANGLE,"--n_sample_out_nuc_",currenti,".csv"))
  write_csv(n_sample_out_cyto, paste0(UnetTrain_dir,"Plate_", Plate, "--Well_", Well, "--Position_", Position,"--Angle_",ANGLE,"--n_sample_out_cyto_",currenti,".csv"))
