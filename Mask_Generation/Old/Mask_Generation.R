...
# Settings#############################################################################################################################################
UnetTrain_dir <- "/home/gendarme/Documents/U-Net/PMI_Data/UnetTrain"

# List of files########################################################################################################################################
AllImages <- as.data.frame(list.files(WD, recursive = T), col.names = 'FileNames')
...
ImageProcessing <- function(Image, Plate, Well, Position){
  ...
  nNuc <- range(nucMask)[2]
  cytoMask <- normalize(cytoMask, inputRange = c(0, nNuc))
  nucMask <- normalize(nucMask)
  ...
  nucMaskList <- list(as.array(abind(nucMask, rev.along = 0)))
  head(AllImages)
  AllImages$Encoding
}
...
# ftsCyto = computeFeatures.moment(cytoMask)
# display(cytoMaskNorm, method="raster")
...
AllImages <- AllImagesSmall[complete.cases(AllImagesSmall),] # Small sample size for testing
