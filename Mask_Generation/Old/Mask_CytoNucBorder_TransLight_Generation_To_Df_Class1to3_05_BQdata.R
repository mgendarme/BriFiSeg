...
# Settings#############################################################################################################################################
UnetTrain_dir <- "~/Documents/UNet/BF_Data/BQ_Data/Train"
base_dir <- "~/Documents/UNet/BF_Data/BQ_Data/TLdataBQ_092019_BioMedX_Testplate02_001"
...
train_data <- tibble(ImageId = ImageTrain$ImageId) %>%
  ...
  mutate(ImageFile = file.path(base_dir, paste0(ImageId, ".tif")),
         ...
         ImageShape = rep(list(c(crop_range, crop_range)), nrow(ImageTrain)),
         Plate = 1,
...
ImageProcessingAndEncoding <- function(Image, Plate, Well, Position){
  ...
  #display(NImg488)
  ImgTL = readImage(train_data[train_data$Channel == "Transmission_0" & Boolean, "ImageFile"])[crop_range, crop_range]
  ImgTL = Rescale(ImgTL)
  # filter_high <- matrix(1, nc = 3, nr = 3)
  # filter_high[2, 2] <- -10
  # Img568_high <- filter2(Img568, filter_high)
   # display(normalize(ImgTL * 1))
  
   ImgTLm3 = readImage(train_data[train_data$Channel == "Transmission_m3" & Boolean, "ImageFile"])[crop_range, crop_range]
   ImgTLm6 = readImage(train_data[train_data$Channel == "Transmission_m6" & Boolean, "ImageFile"])[crop_range, crop_range]
   ImgTLp3 = readImage(train_data[train_data$Channel == "Transmission_p3" & Boolean, "ImageFile"])[crop_range, crop_range]
   ImgTLp6 = readImage(train_data[train_data$Channel == "Transmission_p6" & Boolean, "ImageFile"])[crop_range, crop_range]
  ...
  nucThrManual = thresh(Img405smooth, w = 100, h = 100, offset = 0.1)
  ...
  n_sample_nuc <- as.tibble(train_data[Boolean & train_data$Channel == "dapi",]) %>% 
    ...
    mutate(BorderEncodedPixels = map2(BorderMasks, ImageShape, postprocess_image),
           ...
           # InterfaceEncodedPixels = ifelse(length(InterfaceEncodedPixels[[1]] != 1), list(""), InterfaceEncodedPixels),
  ...
  # str(n_sample_nuc)
  # length(n_sample_nuc$InterfaceEncodedPixels[[1]])
  # length(n_sample_nuc$MaskEncodedPixels[[1]])
  ...
  ## Encoding cytoplasm ##################################################################################
  n_sample_cyto <- as.tibble(train_data[Boolean & train_data$Channel == "Transmission_0",]) %>% 
    add_column(BorderMasks = list(as.array(ifelse(InterfaceBorderNuc == 2, 1, 0))),
               InterfaceMasks = list(as.array(ifelse(InterfaceBorderNuc == 3, 1, 0))),
...
train_data <- train_data[!(train_data$ID %in% c("A17", "F1", "K1", "K3")), ] # InterfaceEncodedPicels has an issue ("A17", "F1", "K1", "K3")
