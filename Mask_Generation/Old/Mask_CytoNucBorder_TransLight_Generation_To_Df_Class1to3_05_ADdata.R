...
# Settings#############################################################################################################################################
UnetTrain_dir <- "~/Documents/UNet/BF_Data/Train"
base_dir <- "~/Documents/UNet/BF_Data/AD_2019_022"
...
ImageTrain <- filter(ImageMaskTrain, str_detect(ImageId, "c1|c5"))#|NImg405|Img568|Img405"))
...
train_data <- data.frame(ImageId = ImageTrain$ImageId) %>%
  ...
  mutate(ImageFile = file.path(base_dir, paste0(ImageId, ".tif")),
         ...
         Plate = ImageId,
         Plate = sub(".*_Plate", "", Plate),
         Plate = as.numeric(sub("_Well.*", "", Plate)),
         ID = sub(".*Well", "", ImageId),
         ID = sub("_Channel.*", "", ID),
         Position = sub(".*xy", "", ImageId),
         Position = sub("c.*", "", Position),
         Channel = sub(".*c", "", ImageId),
         ImageId = paste0("AD_2019_022/",ImageId)
  )
...
ImageProcessingAndEncoding <- function(Image, Plate, Well, Position){
  ...
  #---------------Load Images------------------------
  Img405 = readImage(train_data[train_data$Channel == "1" & Boolean, "ImageFile"])[crop_range, crop_range]
  # str(Img405)
  ...
  #display(NImg488)
  ImgTL = readImage(train_data[train_data$Channel == "5" & Boolean, "ImageFile"])[crop_range, crop_range]
  ...
  ## Encoding nucleus #####################################################################################
  n_sample_nuc <- as.tibble(train_data[Boolean & train_data$Channel == "1",]) %>% 
  ...
  ## Encoding cytoplasm ##################################################################################
  n_sample_cyto <- as.tibble(train_data[Boolean & train_data$Channel == "5",]) %>% 
    add_column(BorderMasks = list(as.array(ifelse(InterfaceBorderNuc == 2, 1, 0))),
               InterfaceMasks = list(as.array(ifelse(InterfaceBorderNuc == 3, 1, 0))),
