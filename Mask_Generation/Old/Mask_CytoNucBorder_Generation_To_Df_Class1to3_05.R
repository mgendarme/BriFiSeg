...
TRAIN_PATH = paste0(base_dir,"/Train--EBImage::normalize-Range_1-2") 
HEIGHT = 1000
WIDTH = 1000
...
train_data <- data.frame(ImageId = ImageTrain$ImageId) %>%
  ...
  mutate(ImageFile = file.path(base_dir, paste0(ImageId, ".tif")),
         ...
         Channel = sub(".*--", "", ImageId),
         ImageId = paste0("IF012_plate001_001/",ImageId)
  )
...
ImageProcessingAndEncoding <- function(Image, Plate, Well, Position){
  ...
  ## Export encoding
  write_csv(n_sample_out_nuc, paste0(UnetTrain_dir,"Plate_", Plate, "--Well_", Well, "--Position_", Position,"--n_sample_out_nuc_",currenti,".csv"))
  write_csv(n_sample_out_cyto, paste0(UnetTrain_dir,"Plate_", Plate, "--Well_", Well, "--Position_", Position,"--n_sample_out_cyto_",currenti,".csv"))
