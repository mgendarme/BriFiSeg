...
# CELL = "BATCH_RPE1"                         # c("THP1", "A549", "RPE1", "HELA", "MDM")
...
currenti = paste0("noBorder_shape", kernel_shape, "_thickness", THICKNESS, "_batch")
...
train_data %>% glimpse()
WELLS = unique(train_data$ID)[1:20]
...
ImageProcessingAndEncoding <- function(Image, Plate, Well, Position, Shape){
  ...
  ##For testing only:
  # Plate = 1
  # Well = ifelse(CELL %in% c("BATCH_RPE1", "BATCH_MCF7"), "E13", "E03")
  # Position = 1
  ...
  ################### Check final product ###################
  # display(Segm405)
  # writeImage(Segm405, files = paste0("/home/gendarme/Desktop/testseg", CELL, ".tiff"), quality = 100, type = "tiff")
  ###########################################################
  
  # segm_dir = str_replace(segm_dir, "~", "C:/Users/Gendarme/Documents")
  ...
  for(Well in unique(train_data[train_data$Plate == Plate, ]$ID)) {
    ...
    # for(Position in unique(train_data[train_data$Plate == Plate &
     ...
     foreach(Position = unique(train_data$Position), .packages = c("EBImage", "tidyverse")) %:%
      ...
      print(paste0(CELL, " || ",
                    "Computation time: ", round((proc.time() - ptm)[3] , 2),
                    " s || Current condition:",  
                    " -- Well: ", Well#,
                    # " -- Position: ", Position
                    ))
