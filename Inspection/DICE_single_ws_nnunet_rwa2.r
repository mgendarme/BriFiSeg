...
for (l in 1:dim(Y_val)[1]) {
  Y_val_label[l,,,] = bwlabel(Y_val[l,,,])
  Y_hat_val_label[l,,,] = pred2label(Y_hat_val[l,,,], thresh = 0.5)
}

# imglbl = (abind(
#     colorLabels(Y_val_label[1,,,2]),
#     colorLabels(Y_hat_val_label[1,,,2]),
#     along=1
# ))
# writeImage(imglbl, "~/Desktop/imglbl.png")

# generate variable in case of empty field of view
fov_empty = dim(Y_val_label)[4]
if(fov_empty == 1){
    Y_val_label = array(0, dim = c(dim(Y_val)[1:3], 2))
    Y_val_label[,150:300,150:300,] = 1
    Y_hat_val_label = Y_val_label
    for (l in 1:dim(Y_val)[1]) {
        Y_val_label[l,,,] = bwlabel(Y_val_label[l,,,])
        Y_hat_val_label[l,,,] = pred2label(Y_hat_val_label[l,,,], thresh = 0.5)
    }
}

Y_val_label = Y_val_label[,,,2]
Y_val_label = add_dim(Y_val_label, dim3 = 1) 
dim(Y_val_label) = c(1, dim(Y_val_label))
Y_hat_val_label = Y_hat_val_label[,,,2]
Y_hat_val_label = add_dim(Y_hat_val_label, dim3 = 1) 
dim(Y_hat_val_label) = c(1, dim(Y_hat_val_label))
