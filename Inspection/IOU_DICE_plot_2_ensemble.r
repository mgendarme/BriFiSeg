...
# Y_hat_test = (predict(model1, X_test, batch_size = pred_batch_size) +
#     predict(model2, X_test, batch_size = pred_batch_size) +
#     predict(model3, X_test, batch_size = pred_batch_size) +
#     predict(model4, X_test, batch_size = pred_batch_size) +
#     predict(model5, X_test, batch_size = pred_batch_size)) / 5
...
Y_hat_test_class_1 = apply(Y_hat_test_class_1, c(1, 2, 3), max)
...
Y_hat_test_class_2 = apply(Y_hat_test_class_2, c(1, 2, 3), max)
...
for(i in ifelse(ACTIVATION == "softmax", 2, 1):ifelse(ACTIVATION == "softmax", CLASS + 1, CLASS)){
    ...
    ## test
    test_iou =  format(round(mean(iou_test_post_proc, na.rm = T), 4), nsmall = 3)
    png(paste0(Save_plot_semantic, "/IOU_3_CLASS_", ifelse(ACTIVATION == "softmax", i-1, i),"_Boxplot_test_postproc_.png"),
        width = 800, height = 1200, res = 300)
...
for(i in ifelse(ACTIVATION == "softmax", 2, 1):ifelse(ACTIVATION == "softmax", CLASS + 1, CLASS)){
    ...
    ## test
    test_dice = format(round(mean(dice_test_post_proc, na.rm = T), 4), nsmall = 3)
    
    print(paste0("test dice = ", test_dice))

    png(paste0(Save_plot_semantic, "/DICE_3_CLASS_", ifelse(ACTIVATION == "softmax", i-1, i),"_Boxplot_test_postproc.png"),
        width = 800, height = 1200, res = 300)
...
write_json_data_index(json_metric, paste0(Save_dir, "/detail_metric_", "ensemble", ".json"))
