...
TRAIN_PATH = paste0(Unet_dir,"/", ifelse(CELL %in% c("THP1", "A549"), CELL, paste0("BATCH_", CELL)) ,"/Train") 
TEST_PATH = paste0(Unet_dir,"/", ifelse(CELL %in% c("THP1", "A549"), CELL, paste0("BATCH_", CELL)),"/Test")
ZSTACK = ifelse(CELL == "THP1", 0, 4) %>%
    seq(., . + 2, 1)
ZSTACK = ZSTACK[2]

all_sample_out <- map_df(list.files(TRAIN_PATH,
                                    pattern = ifelse(CELL == "A549",
                                      paste0(TypeOfObjects, "_noBorder_shapedisc_thickness5_old.csv"),
                                      paste0(TypeOfObjects, "_noBorder_shapedisc_thickness5_batch.csv")),
                                    full.names = TRUE,
                                    recursive = T),
...
data <- all_sample_out %>%
  mutate(ImageFile = file.path(paste0(Unet_dir, "/",CELL,"/Image/", ImageId)),
        ...
        #  Position = as.numeric(sapply(str_split(sapply(str_split(ImageId, "_"), "[", 3), "_Channel"), "[", 1)),
         Position = as.numeric(sapply(str_split(sapply(str_split(ImageId, "_"), "[", ifelse(CELL == "THP1", 3, 5)), "_Position"), "[", 1)),
         Channel = sapply(str_split(sapply(str_split(ImageId, "Channel"), "[", 2), "_"), "[", 1),
         Crop = map(str_split(Crop, " "), as.numeric)
  ) %>% select(-c("Channel")) %>%
  filter(ID %in% c(WELLS)) %>% 
  filter(!is.na(InterfaceEncodedPixels)) %>%
  glimpse

image_data <- tibble(ImageId_BF = list.files(paste0(Unet_dir, "/",CELL,"/Image"), recursive = T)) %>% 
  mutate(ImageFile_BF = file.path(paste0(Unet_dir, "/",CELL,"/Image/", ImageId_BF)),
         Plate = 1,#as.numeric(str_sub(ImageId, 16, 18)),
         ID = sapply(str_split(sapply(str_split(ImageId_BF, "Well"), "[", 2), "_"), "[", 1),
         Position = as.numeric(sapply(str_split(sapply(str_split(ImageId_BF, "_"), "[", ifelse(CELL == "THP1", 3, 5)), "_Channel"), "[", 1)),
...
if(length(ZSTACK) == 1){
  input <- data %>% 
    mutate(X = map2(ImageFile_, Crop, ~ preprocess_image_TL(.x, shape = c(1000, 1000), cropping = .y)),
           ...
           I = map_if(InterfaceEncodedPixels, InterfaceEncodedPixels != "1 1000000", rle2masks, c(1000, 1000)), #cropping = HEIGHT),)
           I = map_if(I, InterfaceEncodedPixels == "1 1000000", ~ array(data = 0, dim = c(1000, 1000)))) %>%
        select(X, Y, B, I, ID)
} else if(length(ZSTACK) == 3){
