...
TRAIN_PATH = paste0(Unet_dir,"/RWA_",CELL,"/Train") 
TEST_PATH = paste0(Unet_dir,"/RWA_",CELL,"/Test")
...
un_w = unique(sapply(str_split(sapply(str_split(all_sample_out$ImageId, "Well"), "[", 2), "_"), "[", 1))
WELLS = un_w[21:length(un_w)]
...
make_correct_lbl_annika = function(str_in){
  # old_str = data$ImageFile_BF[[1]]
  old_str = str_in
  old_str_short = str_split(old_str, "Seq")[[1]][1]
  Seq = as.numeric(str_replace(str_split(old_str, "Seq")[[1]][2], ".tiff", ""))
  new_Seq = as.character(Seq + 2)
  if(nchar(new_Seq) == 3){
    new_Seq = paste0("Seq", 0, new_Seq)
  } else if(nchar(new_Seq) == 2){
    new_Seq = paste0("Seq", 0, 0, new_Seq)
  } else if(nchar(new_Seq) == 1){
    new_Seq = paste0("Seq", 0, 0, 0, new_Seq)
  }
  new_str = paste0(old_str_short, new_Seq, ".tiff")
  return(new_str)
}
# make_correct_lbl_annika(data$ImageFile_BF[[1]])

data <- all_sample_out %>%
  mutate(ImageFile = file.path(paste0(Unet_dir, "/RWA_",CELL,"/Image/", ImageId)),
        ...
        #  Position = as.numeric(sapply(str_split(sapply(str_split(ImageId, "_"), "[", 3), "_Channel"), "[", 1)),
         Position = as.numeric(sapply(str_split(ImageId, "_"), "[", 7)),
         Channel = sapply(str_split(sapply(str_split(ImageId, "Channel"), "[", 2), "_"), "[", 1),
         Crop = map(str_split(Crop, " "), as.numeric),

         ImageFile_BF = str_replace(ImageFile, 'DAPI', 'DIA'),
         ImageFile_BF = map(ImageFile_BF, make_correct_lbl_annika)
  ) %>% 
  # select(-c("Channel")) %>%
...
# data$ImageFile_BF[[10]]
# data$ImageFile[[10]]

# data$ImageId
# data$Position
# sapply(str_split(sapply(str_split(data$ImageId, "Channel"), "[", 2), "_"), "[", 1)
# data$Channel
# data$ID

image_data <- tibble(ImageId_BF = list.files(paste0(Unet_dir, "/RWA_",CELL,"/Image"), recursive = T)) %>% 
  mutate(ImageFile_BF = file.path(paste0(Unet_dir, "/RWA_",CELL,"/Image/", ImageId_BF)),  
         Plate = 1,#as.numeric(str_sub(ImageId, 16, 18)),
       ID = sapply(str_split(sapply(str_split(ImageId_BF, "Well"), "[", 2), "_"), "[", 1),
         Position = as.numeric(sapply(str_split(ImageId_BF, "_"), "[", 7)),
