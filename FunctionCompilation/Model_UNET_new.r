...
build_unet = function(input_shape,
  ...
  if(is.null(backbone)){
    ...
    if(str_detect(backbone, "^resnet") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_ResNet.r"))
    } else if(str_detect(backbone, "efficientnet") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_EfficientNet.r"))
    } else if(str_detect(backbone, "inception_resnet_v2") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_InceptionResnetv2_padding_same.r"))
    } else if(str_detect(backbone, "seresnext") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_SeNet.r"))
    } else if(str_detect(backbone, "senet") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_SeNet.r"))
    } else if(str_detect(backbone, "resnext") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_ResNeXt.r"))
    } else if(str_detect(backbone, "Xception") == TRUE){
      source(paste0(RelPath, "/FunctionCompilation/Backbone_Xception_padding_same.r"))
    }
