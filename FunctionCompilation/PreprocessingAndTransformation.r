...
crop_mut = function(image, cropping){
  if(!is.na(cropping) | is.na(cropping[[1]])){
...
preprocess_image_TL_3c <- function(file1, file2, file3, shape, cropping = NULL, rescaling = NULL){
  image = array(0, dim = shape)
  #  image = array(0, dim = c(1000, 1000, 3))
  #  cropping = train_data$Crop[[1]]
  # rescaling = NULL
  # str(image)
  ...
  #  str(image)
  #  display(normalize(image[,,1]))
  #  display(filter2(normalize(image[,,1]), sharp))
  #  display(abind(normalize(image[,,1]),
  #                normalize(filter2(image[,,1], sharp)),
  #                along = 1),
  #                bg = "black")

  # display(readImage(file1, type = "tiff"))
  # a = readImage(file1, type = "tiff")
  # str(a)
  # b = readImage(file1, type = "tiff")
  # b = b[cropping[[1]][1]:cropping[[1]][2], cropping[[1]][3]:cropping[[1]][4]]
  # str(b)
  # image <- Rescale(image)
  # image <- to_gray_scale(image)                        ## convert to gray scale  
